...
template <typename T> struct AVLNode : public INode {
    ...
    constexpr explicit AVLNode(TT&& t_key) : m_key(std::forward<TT>(t_key)) {}

    template <typename TT>
    constexpr AVLNode(TT&& t_key, AVLNode* t_left, AVLNode* t_right)
        : m_key(std::forward<TT>(t_key)), m_left(t_left), m_right(t_right) {}
  ...
  public:
    AVLNode& operator=(AVLNode<T>&& t_node) noexcept {
        ...
        if (this != &t_node) {
            m_key = std::exchange(t_node.m_val, T());
